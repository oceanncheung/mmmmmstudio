...
(function () {
  ...
  function applyDeferredPoster(element) {
  ...
  function isContinuousEmbed(element) {
    return Boolean(element && element.tagName === 'IFRAME' &&
      element.getAttribute('data-embed-kind') === 'v7-cup');
  }
  var mediaActiveState = new WeakMap();
  function activateDeferred(element) {
    if (!element || element.dataset.mmsLoaded === '1') return;
    if (motionIsReduced() &&
        (element.tagName === 'VIDEO' || isContinuousEmbed(element))) {
      applyDeferredPoster(element);
      return;
    }
  ...
  function requestVideoPlay(video) {
    if (!video || video.dataset.mmsLoaded !== '1' ||
        motionIsReduced() || document.hidden) return;
    video.muted = true;
  ...
  function setMediaActive(element, active) {
    mediaActiveState.set(element, Boolean(active));
    if (motionIsReduced() &&
        (element.tagName === 'VIDEO' || isContinuousEmbed(element))) {
      applyDeferredPoster(element);
      if (element.tagName === 'VIDEO') {
        try { element.pause(); } catch (e) {}
      } else if (element.dataset.mmsLoaded === '1') {
        postEmbedVisibility(element, false);
      }
      return;
    }
  ...
  /* Native GIFs have no playback API. Under Reduce Motion, take one
     same-size canvas snapshot in place, then release the animated source.
     Normal mode never creates or displays the snapshot and keeps the exact
     approved GIF presentation. */
  var motionGifState = new WeakMap();
  function freezeMotionGif(image) {
    if (!image || !motionIsReduced() || motionGifState.has(image)) return;
    if (!image.complete || !image.naturalWidth || !image.naturalHeight) return;
    var frame = image.closest('.mms-frame');
    if (!frame || !frame.clientWidth || !frame.clientHeight) return;
    var canvas = frame.querySelector('canvas.mms-reduced-motion-still');
    if (!canvas) {
      canvas = document.createElement('canvas');
      canvas.className = 'mms-reduced-motion-still';
      canvas.setAttribute('aria-hidden', 'true');
      frame.appendChild(canvas);
    }
    var cssWidth = frame.clientWidth;
    var cssHeight = frame.clientHeight;
    ...
    var fit = frame.getAttribute('data-fit') === 'cover' ? 'cover' : 'contain';
    var scale = fit === 'cover'
      ? Math.max(cssWidth / image.naturalWidth, cssHeight / image.naturalHeight)
      : Math.min(cssWidth / image.naturalWidth, cssHeight / image.naturalHeight);
    ...
    motionGifState.set(image, { source: image.getAttribute('src') || '' });
    image.classList.add('is-motion-frozen');
    image.style.visibility = 'hidden';
    canvas.hidden = false;
    image.removeAttribute('src');
  }
  function restoreMotionGif(image) {
    if (!image) return;
    var state = motionGifState.get(image);
    if (!state) return;
    var frame = image.closest('.mms-frame');
    var canvas = frame && frame.querySelector('canvas.mms-reduced-motion-still');
    if (canvas) canvas.hidden = true;
    image.style.removeProperty('visibility');
    image.classList.remove('is-motion-frozen');
    motionGifState.delete(image);
    if (state.source) image.setAttribute('src', state.source);
  }
  function syncMotionGifs() {
    currentMms.querySelectorAll('img[data-motion-kind="gif-loop"]').forEach(function (image) {
      if (!motionIsReduced()) {
        restoreMotionGif(image);
        return;
      }
      if (image.complete && image.naturalWidth) freezeMotionGif(image);
    });
  }
  currentMms.querySelectorAll('img[data-motion-kind="gif-loop"]').forEach(function (image) {
    runtime.on(image, 'load', function () {
      if (motionIsReduced()) runtime.raf(function () { freezeMotionGif(image); });
    });
  });
  ...
  runtime.on(window, 'message', function (event) {
    ...
    frames.forEach(function (frame) {
      ...
      postEmbedVisibility(
        frame,
        mediaActiveState.get(frame) === true && !motionIsReduced() && !document.hidden
      );
    });
  });
  startDeferredMedia();
  ...
  function resumeActiveMotion() {
    ...
    currentMms.querySelectorAll('video[data-src], iframe[data-src]').forEach(function (element) {
      if (mediaActiveState.get(element) === true) setMediaActive(element, true);
    });
  }
  function syncReducedMotion(changeEvent) {
    syncMotionGifs();
    if (motionIsReduced()) {
      pauseOwnedMotion();
      currentMms.querySelectorAll('video[data-poster], iframe[data-embed-kind="v7-cup"]')
        .forEach(applyDeferredPoster);
      return;
    }
    if (changeEvent) resumeActiveMotion();
  }
  runtime.onMql(reducedMotionQuery, syncReducedMotion);
  syncReducedMotion();
  runtime.cleanup(function () {
    currentMms.querySelectorAll('img[data-motion-kind="gif-loop"]').forEach(function (image) {
      restoreMotionGif(image);
    });
    currentMms.querySelectorAll('canvas.mms-reduced-motion-still').forEach(function (canvas) {
      canvas.remove();
    });
  });
